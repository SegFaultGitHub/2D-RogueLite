...
namespace Code.Enhancements.UnlockConditions.Runtime.Predicates {
    [Serializable]
    public class C_MaxDamageReceivedFromSpecificSource : C_MaxDamageReceived {
        [field: SerializeReference] public E_DamageSource Source { get; set; }

        public C_MaxDamageReceivedFromSpecificSource(E_Mode mode, float damage, E_DamageSource source) : base(mode, damage) {
            this.Source = source;
        }

        public override bool Check(MB_ObjectsManager objectsManager) =>
            this.Mode switch {
                E_Mode.CurrentRun => objectsManager.StatsManager.CurrentRunStats.GetMaxDamageReceived(this.Source) >= this.Damage,
                E_Mode.Global => objectsManager.StatsManager.GlobalStats.GetMaxDamageReceived(this.Source) >= this.Damage,
        ...
        public override void GetVerbose(
            ...
            string current = SC_Utils.FormatNumber(objectsManager.StatsManager.GlobalStats.GetMaxDamageReceived(this.Source));
            ...
            //return $"{indentString}- Receive {goal} {this.Source} damage at once";

            unlockConditions.Add(
                new C_UnlockCondition {
                    Indent = indent,
                    Text = $"Receive {goal} {this.Source} damage at once",
