...
namespace Code.Enhancements.UnlockConditions.Runtime.Predicates {
    ...
    public class C_MaxDamageReceived : C_Predicate {
        ...
        public C_MaxDamageReceived(E_Mode mode, float damage) : base(mode) {
            this.Damage = damage;
        }

        public override bool Check(MB_ObjectsManager objectsManager) =>
            this.Mode switch {
                E_Mode.CurrentRun => objectsManager.StatsManager.CurrentRunStats.GetMaxDamageReceived() >= this.Damage,
                E_Mode.Global => objectsManager.StatsManager.GlobalStats.GetMaxDamageReceived() >= this.Damage,
