...
namespace Code.Enhancements.UnlockConditions.Runtime.Predicates {
    ...
    public class C_DamageDealtToSpecificEnemies : C_DamageDealt {
        ...
        public C_DamageDealtToSpecificEnemies(E_Mode mode, float damage, E_Enemy enemy) : base(mode, damage) {
            this.Enemy = enemy;
        }

        public override bool Check(MB_ObjectsManager objectsManager) =>
            this.Mode switch {
                E_Mode.CurrentRun => objectsManager.StatsManager.CurrentRunStats.GetDamageDealt(this.Enemy) >= this.Damage,
                E_Mode.Global => objectsManager.StatsManager.GlobalStats.GetDamageDealt(this.Enemy) >= this.Damage,
        ...
        public override void GetVerbose(
            ...
            string current = SC_Utils.FormatNumber(objectsManager.StatsManager.GlobalStats.GetDamageDealt(this.Enemy));
            ...
            //return $"{indentString}- Deal {goal} damage to {this.Enemy}";

            unlockConditions.Add(
                new C_UnlockCondition {
                    Indent = indent,
                    Text = $"Deal {goal} damage to {this.Enemy}",
