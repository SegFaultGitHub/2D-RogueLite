...
namespace Code.Spells.Enemies {
    public abstract class AMB_Summon : AMB_PositionalSpell {
        #region Members
        [Foldout("AMB_Summon", true)]
        ...
        public override void SetPositions(Vector2 from, Vector2 to) {
            ...
            MB_SummonSpawner spawner = Instantiate(this.Spawner, this.ObjectsManager.RoomManager.Room.EnemiesParent);
            spawner.transform.position = to;
            spawner.Enemies = this.Enemies;
            spawner.Summoner = this.Character as AMB_Enemy;
