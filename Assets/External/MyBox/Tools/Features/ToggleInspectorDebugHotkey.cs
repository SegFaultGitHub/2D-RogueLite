    ...
    public static class ToggleInspectorDebugHotkey {
        ...
        static void ToggleInspectorDebug() {
            ...
            if (_inspectorWindow != null && _inspectorWindow.GetType().Name == "InspectorWindow") {
                FieldInfo inspectorMode = inspectorWindowType.GetField("m_InspectorMode", BindingFlags.NonPublic | BindingFlags.Instance);
                MethodInfo setModeMethod = inspectorWindowType.GetMethod("SetMode", BindingFlags.NonPublic | BindingFlags.Instance);

                if (inspectorMode == null || setModeMethod == null) return;

                InspectorMode mode = (InspectorMode)inspectorMode.GetValue(_inspectorWindow);
                mode = mode == InspectorMode.Normal
                    ? InspectorMode.Debug
