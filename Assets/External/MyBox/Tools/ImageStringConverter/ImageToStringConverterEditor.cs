    ...
    public class ImageToStringConverterEditor : EditorWindow {
        ...
        private void OnGUI() {
            ...
            EditorGUILayout.HelpBox(
                "1: Drag any image to \"Drag Texture\" field below \n"
                ...
                + "	\"ImageStringConverter.ImageFromString\"",
                MessageType.Info
            );
            EditorGUILayout.Space();
            ...
            GUI.enabled = false;
            EditorGUILayout.TextArea(this._representation);
            GUI.enabled = true;

            EditorGUILayout.Space();
            using (new EditorGUILayout.HorizontalScope()) {
                EditorGUILayout.LabelField("Preview Width: ", GUILayout.Width(120));
                this._width = EditorGUILayout.IntField(GUIContent.none, this._width, GUILayout.Width(60));
                EditorGUILayout.LabelField("Preview Height: ", GUILayout.Width(120));
                this._height = EditorGUILayout.IntField(GUIContent.none, this._height, GUILayout.Width(60));
            }

            if (this._texture != null) EditorGUILayout.LabelField(new GUIContent(this._texture), GUILayout.Width(this._width), GUILayout.Height(this._height));

            if (selected == null || selected.Length == 0) return;

            string content = Convert.ToBase64String(File.ReadAllBytes(selected[0]));
            this._representation = content;

            MyEditor.CopyToClipboard(this._representation);
            this.ShowNotification(new GUIContent(selected[0] + "\nCopied to Clipboard as string"));

            this._texture = ImageStringConverter.ImageFromString(this._representation, this._width, this._height);
        }
    }
}
#endif
