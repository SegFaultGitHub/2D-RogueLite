...
namespace MyBox.Internal {
    ...
    public class MyBoxWindow : EditorWindow {
        ...
        private static void CheckForUpdates() {
            ...
            MyBoxUtilities.GetMyBoxLatestVersionAsync(
                version => {
        ...
        private void InitializeStyles() {
            if (this._titleStyle == null) {
                this._titleStyle = new GUIStyle(EditorStyles.boldLabel);
                this._titleStyle.fontSize = 42;
                this._titleStyle.fontStyle = FontStyle.Bold;
                this._titleStyle.alignment = TextAnchor.MiddleCenter;
            }

            if (this._buttonStyle == null) {
                this._buttonStyle = new GUIStyle(MyGUI.HelpBoxStyle);
                this._buttonStyle.normal.textColor = GUI.skin.textField.normal.textColor;
                this._buttonStyle.hover.textColor = EditorGUIUtility.isProSkin
                    ? Color.white
                    : MyGUI.Colors.Gray;
            }

            if (this._sponsorButtonStyle == null) {
                this._sponsorButtonStyle = new GUIStyle(this._buttonStyle);
                this._sponsorButtonStyle.normal.textColor = EditorStyles.centeredGreyMiniLabel.normal.textColor;
                this._sponsorButtonStyle.fontSize *= 2;
                this._sponsorButtonStyle.fontStyle = FontStyle.Bold;
            }

            if (this._externalDocIcon == null) {
                this._externalDocIcon = new GUIContent(MyGUI.EditorIcons.Help);
                this._externalDocIcon.tooltip = "Open external documentation " + UrlSymbol;
            }

            if (this._externalDocStyle == null) {
                this._externalDocStyle = new GUIStyle(EditorStyles.iconButton);
                this._externalDocStyle.margin = new RectOffset(0, 0, 3, 0);
            }
        }
        ...
        private void OnGUI() {
            GUI.enabled = this.GuiEnabledGlobal;
            ...
            EditorGUILayout.LabelField("MyBox", this._titleStyle, GUILayout.Height(60));

            using (new EditorGUILayout.HorizontalScope()) {
                GUILayout.FlexibleSpace();

                if (GUILayout.Button("  Github Page " + UrlSymbol, this._buttonStyle, buttonWidth, buttonHeight))
                    Application.OpenURL("https://github.com/Deadcows/MyBox");

                if (GUILayout.Button("  Attributes " + UrlSymbol, this._buttonStyle, buttonWidth, buttonHeight))
                    Application.OpenURL("https://github.com/Deadcows/MyBox/wiki/Attributes");

                if (GUILayout.Button("  Extensions " + UrlSymbol, this._buttonStyle, buttonWidth, buttonHeight))
                    Application.OpenURL("https://github.com/Deadcows/MyBox/tree/master/Extensions");

                if (GUILayout.Button("  Tools, Features " + UrlSymbol, this._buttonStyle, buttonWidth, buttonHeight))
            ...
            using (new EditorGUILayout.HorizontalScope()) {
                ...
                using (new EditorGUILayout.VerticalScope()) {
                    ...
                    using (new EditorGUILayout.HorizontalScope()) {
                        this.DrawInspectorOverrideSetting();
                        GUILayout.FlexibleSpace();
                    }

                    using (new EditorGUILayout.HorizontalScope()) {
                        this.DrawUnityEventOverrideSetting();
                        GUILayout.FlexibleSpace();
                    }
                    ...
                    using (new EditorGUILayout.HorizontalScope()) {
                        ...
                        MyBoxSettings.CleanEmptyDirectoriesFeature = EditorGUILayout.Toggle(
                            label,
                ...
                using (new EditorGUILayout.VerticalScope()) {
                    ...
                    using (new EditorGUILayout.HorizontalScope()) {
                        ...
                        if (GUILayout.Button(this._externalDocIcon, this._externalDocStyle, GUILayout.Height(18)))
            ...
            using (new EditorGUILayout.HorizontalScope()) {
                var current = _installedVersion == null
                    ? "..."
                    : _installedVersion.AsSting;
                var latest = _latestVersion == null
                    ? "..."
                    : _latestVersion.AsSting;
                var installationType = MyBoxUtilities.InstalledViaUPM
                    ? "UPM"
                    : "(not UPM)";
            ...
            using (new EditorGUILayout.HorizontalScope()) {
                ...
                GUI.enabled = this.GuiEnabledGlobal && (this._updateRequest == null || this._updateRequest.IsCompleted);
                var updateOrInstall = MyBoxUtilities.InstalledViaUPM
                    ? "Update"
                    : "Install";
                if (GUILayout.Button(updateOrInstall + " UPM version", this._buttonStyle, buttonWidth, buttonHeight)) {
                    if (MyBoxUtilities.InstalledViaUPM) AddPackage();
                        ...
                        if (EditorUtility.DisplayDialog(
                                ...
                                "Ok, Install UPM version!",
                                "Nah, keep it as it is"
                            )) AddPackage();
                    }

                    void AddPackage() {
                        this._updateRequest = UnityEditor.PackageManager.Client.Add("https://github.com/Deadcows/MyBox.git");
                    }
                }

                GUI.enabled = this.GuiEnabledGlobal;

                if (GUILayout.Button("  How to Update " + UrlSymbol, this._buttonStyle, buttonWidth, buttonHeight))
                    Application.OpenURL("https://github.com/Deadcows/MyBox/wiki/Installation");

                if (GUILayout.Button("  Releases " + UrlSymbol, this._buttonStyle, buttonWidth, buttonHeight))
                    Application.OpenURL("https://github.com/Deadcows/MyBox/releases");

                if (GUILayout.Button("  Changelog " + UrlSymbol, this._buttonStyle, buttonWidth, buttonHeight))
            ...
            using (new EditorGUILayout.HorizontalScope()) {
                GUILayout.FlexibleSpace();
                if (GUILayout.Button("support my work \u2665", this._sponsorButtonStyle, GUILayout.Width(260), GUILayout.Height(42)))
        ...
        private void DrawInspectorOverrideSetting() {
            ...
            if (EditorGUI.EndChangeCheck()
                && EditorUtility.DisplayDialog(
                    "Toggle \"" + define + "\" define symbol",
                    "This change will cause recompilation, continue?",
                    "Ok",
                    "Cancel"
                )) {
