...
namespace MyBox {
    public class TimeTest : IDisposable {
        ...
        public TimeTest(string title, bool useMilliseconds = false) {
            this._disposableTest = title;
            _tests[this._disposableTest] = new TimeTestData(title, useMilliseconds);
        }

        public void Dispose() {
            _tests[this._disposableTest].EndTest();
            _tests.Remove(this._disposableTest);
        }
        #endregion
        ...
        private struct TimeTestData {
            ...
            public void EndTest() {
                ...
                var elapsedVal = this._precise
                    ? ms
                    : ms / 1000f;
                var valMark = this._precise
                    ? "ms"
                    : "s";

                StringBuilder.Length = 0;
                StringBuilder.Append("Time Test <color=brown>").Append(this._testTitle).Append("</color>: ").Append(elapsedVal).Append(valMark);
