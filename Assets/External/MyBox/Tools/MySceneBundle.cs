...
namespace MyBox.Internal {
    internal class SceneBundle {
        ...
        internal SceneBundle() {
            this.stringData = new Bundle<string>();
            this.floatData = new Bundle<float>();
            this.intData = new Bundle<int>();
            this.boolData = new Bundle<bool>();
            this.objectData = new Bundle<object>();
        }
    ...
    internal class Bundle<T> {
        ...
        internal Bundle() {
            this.bundleData = new Dictionary<string, T>();
        }
        ...
        internal void AddData(string dataKey, T data, bool overrideIfExists = true) {
            bool haveDataWithSameKey = this.bundleData.ContainsKey(dataKey);

            if (haveDataWithSameKey && overrideIfExists) {
                this.bundleData[dataKey] = data;
            } else if (!haveDataWithSameKey) {
                this.bundleData.Add(dataKey, data);
            }
        }

        internal void AddData(KeyValuePair<string, T> keyValuePair, bool overrideIfExists = true) {
            bool haveDataWithSameKey = this.bundleData.ContainsKey(keyValuePair.Key);

            if (haveDataWithSameKey && overrideIfExists) {
                this.bundleData[keyValuePair.Key] = keyValuePair.Value;
            } else if (!haveDataWithSameKey) {
                this.bundleData.Add(keyValuePair.Key, keyValuePair.Value);
            }
        }

        internal bool TryGetData(string dataKey, out T result) {
            return this.bundleData.TryGetValue(dataKey, out result);
        }
        ...
        internal bool DataExists(string dataKey) {
            return this.bundleData.ContainsKey(dataKey);
        }

        internal Dictionary<string, T> GetBundleData() {
            return new Dictionary<string, T>(new Dictionary<string, T>(this.bundleData));
        }
        ...
        internal void AddBundleData(Dictionary<string, T> bundle, bool overrideIfExists) {
            foreach (var data in bundle) {
                this.AddData(data, overrideIfExists);
            }
        }
        ...
        internal void AddBundleData(Bundle<T> bundle, bool overrideIfExists) {
            this.AddBundleData(bundle.bundleData, overrideIfExists);
        }
    }
}
