...
namespace MyBox.Internal {
    ...
    public class AssetsPresetPreprocessEditor : Editor {
        ...
        private void OnEnable() {
            this._labelStyle = new GUIStyle(EditorStyles.label);
            this._labelStyle.richText = true;

            this._target = this.target as AssetsPresetPreprocessBase;

            this._presets = this.serializedObject.FindProperty("Presets");
            this._exclude = this.serializedObject.FindProperty("ExcludeProperties");
            this._reorderableBase = new ReorderableCollection(this._presets);

            this._reorderableBase.CustomDrawerHeight += this.PresetDrawerHeight;
            this._reorderableBase.CustomDrawer += this.PresetDrawer;
            this._reorderableBase.CustomAdd += this.CustomAdd;
        }

        private void OnDisable() {
            if (this._reorderableBase == null) return;
            this._reorderableBase.CustomDrawerHeight -= this.PresetDrawerHeight;
            this._reorderableBase.CustomDrawer -= this.PresetDrawer;
            this._reorderableBase.CustomAdd -= this.CustomAdd;
            this._reorderableBase = null;
        }
        ...
        private bool CustomAdd(int index) {
            EditorApplication.delayCall += () => {
                var newElement = this._presets.GetArrayElementAtIndex(index);
        ...
        private void PresetDrawer(SerializedProperty property, Rect rect, int index) {
            ...
            this.DrawPresetColourLine(rect, properties.Preset.objectReferenceValue as Preset);
            rect.width -= 6;
            rect.x += 6;
        ...
        private struct PresetProperties {
            ...
            public PresetProperties(SerializedProperty baseProperty) {
                this.PathContains = baseProperty.FindPropertyRelative("PathContains");
                this.TypeOf = baseProperty.FindPropertyRelative("TypeOf");
                this.Prefix = baseProperty.FindPropertyRelative("Prefix");
                this.Postfix = baseProperty.FindPropertyRelative("Postfix");
                this.Preset = baseProperty.FindPropertyRelative("Preset");
            }
        }
        ...
        public override void OnInspectorGUI() {
            ...
            EditorGUILayout.LabelField("Assets/...<b>[PC:Path Contains]</b>.../", this._labelStyle);
            EditorGUILayout.LabelField("<b>[Pr:Prefix]</b>...<b>[Po:Postfix]</b>.<b>[FT:File Type]</b>", this._labelStyle);
            EditorGUILayout.Space();

            this._scrollPos = GUILayout.BeginScrollView(this._scrollPos);

            this._reorderableBase.Draw();

            EditorGUILayout.Space();

            EditorGUI.BeginChangeCheck();
            EditorGUILayout.PropertyField(this._exclude, true);
            if (EditorGUI.EndChangeCheck()) {
                EditorApplication.delayCall += this.UpdateExcludes;
            }

            EditorGUILayout.Space();

            if (GUILayout.Button("Update Excludes", EditorStyles.toolbarButton))
                this.UpdateExcludes();
        ...
        private void UpdateExcludes() {
            foreach (var preset in this._target.Presets) {
                if (preset.Preset == null) continue;

                this.UpdateExcludesOnPreset(preset);
            }
        }
        ...
        private void UpdateExcludesOnPreset(ConditionalPreset preset) {
            ...
            foreach (var modification in preset.Preset.PropertyModifications) {
                ...
                bool exclude = false;
                for (var i = 0; i < this._target.ExcludeProperties.Length; i++) {
                    var excludePath = this._target.ExcludeProperties[i];
