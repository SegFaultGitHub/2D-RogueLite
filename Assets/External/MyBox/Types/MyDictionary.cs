...
namespace MyBox {
    ...
    public class MyDictionary<TKey, TValue> : IDictionary<TKey, TValue> {
        ...
        public MyDictionary(IDictionary<TKey, TValue> dictionary, IEqualityComparer<TKey> comparer) : this(
            (dictionary != null)
                ? dictionary.Count
                : 0,
            comparer
        ...
        public ICollection<TKey> Keys {
            get {
                return this._keys.Take(this.Count).ToArray();
            }
        }

        public ICollection<TValue> Values {
            get {
                return this._values.Take(this.Count).ToArray();
            }
        }
            ...
            public bool MoveNext() {
                if (this._Version != this._dictionary._version)
                    throw new InvalidOperationException(
                        string.Format("Enumerator version {0} != Dictionary version {1}", this._Version, this._dictionary._version)
                    );

                while (this._index < this._dictionary._count) {
                    if (this._dictionary._hashCodes[this._index] >= 0) {
                        this.Current = new KeyValuePair<TKey, TValue>(this._dictionary._keys[this._index], this._dictionary._values[this._index]);
                        this._index++;
                        return true;
                    }
