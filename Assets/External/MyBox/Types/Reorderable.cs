    ...
    public class ReorderableTypePropertyDrawer : PropertyDrawer {
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label) {
            if (this._reorderable == null)
                this._reorderable = new ReorderableCollection(property.FindPropertyRelative("Collection"), true, true, property.displayName);

            return this._reorderable != null
                ? this._reorderable.Height
                : base.GetPropertyHeight(property, label);
        }

        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            EditorGUI.BeginProperty(position, label, property);
            this._reorderable.Draw(position);
