...
namespace MyBox {
    ...
    public class AnimationStateReference {
        public string StateName => this._stateName;

        public bool Assigned => this._assigned;

        public Animator Animator => this._linkedAnimator;
    ...
    public class AnimationStateReferenceDrawer : PropertyDrawer {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            string[] GetStatesCollection() {
                if (animatorProperty.objectReferenceValue == null) return this._empty;
                var animator = (Animator)animatorProperty.objectReferenceValue;
                var controller = animator.runtimeAnimatorController as AnimatorController;
                if (controller == null) {
                    var overrideController = animator.runtimeAnimatorController as AnimatorOverrideController;
                    if (overrideController == null) return this._empty;

                    controller = overrideController.runtimeAnimatorController as AnimatorController;
                    if (controller == null) return this._empty;
                }


                var statesInAnimator = controller.layers.SelectMany(l => l.stateMachine.states).Select(s => (s.state.name)).Distinct();
                return this._empty.Concat(statesInAnimator).ToArray();
            }
        }
