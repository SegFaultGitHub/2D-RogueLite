...
namespace MyBox {
    ...
    public class GuidManager {
        ...
        private struct GuidInfo {
            ...
            public GuidInfo(GuidComponent comp) : this() {
                this.GameObject = comp.gameObject;
                this.OnRemove = null;
                this.OnAdd = null;
            }

            public void HandleAddCallback() => this.OnAdd?.Invoke(this.GameObject);

            public void HandleRemoveCallback() => this.OnRemove?.Invoke();
        }
        ...
        private bool InternalAdd(GuidComponent guidComponent) {
            ...
            if (!this._guidToObjectMap.ContainsKey(guid)) {
                this._guidToObjectMap.Add(guid, info);
                return true;
            }

            GuidInfo existingInfo = this._guidToObjectMap[guid];
            if (existingInfo.GameObject != null && existingInfo.GameObject != guidComponent.gameObject) {
                ...
                if (Application.isPlaying) {
                    Debug.AssertFormat(
                        ...
                        (this._guidToObjectMap[guid].GameObject != null
                            ? this._guidToObjectMap[guid].GameObject.name
                            : "NULL"),
                        (guidComponent != null
                            ? guidComponent.name
                            : "NULL")
            ...
            this._guidToObjectMap[guid] = existingInfo;
            return true;
        }

        private void InternalRemove(Guid guid) {
            if (this._guidToObjectMap.TryGetValue(guid, out var info)) {
                // trigger all the destroy delegates that have registered
                info.HandleRemoveCallback();
            }

            this._guidToObjectMap.Remove(guid);
        }
        ...
        private GameObject ResolveGuidInternal(Guid guid, Action<GameObject> onAddCallback, Action onRemoveCallback) {
            if (this._guidToObjectMap.TryGetValue(guid, out var info)) {
                ...
                this._guidToObjectMap[guid] = info;
