...
namespace MyBox {
    ...
    public class DisplayInspectorAttribute : PropertyAttribute {
        ...
        public DisplayInspectorAttribute(bool displayScriptField = true) {
            this.DisplayScript = displayScriptField;
        }
    }
}
...
namespace MyBox.Internal {
    ...
    public class DisplayInspectorAttributeDrawer : PropertyDrawer {
        ...
        private SerializedObject GetTargetSO(Object targetObject) {
            ...
            if (this._targets.ContainsKey(targetObject)) target = this._targets[targetObject];
            else {
                this._targets.Add(targetObject, new SerializedObject(targetObject));
                target = this._targets[targetObject];
            }
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            ...
            bool displayScript = ((DisplayInspectorAttribute)this.attribute).DisplayScript;
            if (displayScript || property.objectReferenceValue == null) {
                ...
                if (property.objectReferenceValue != null) {
                    ...
                    this._foldout.Value = EditorGUI.Foldout(foldRect, this._foldout.Value, label, true);
                    var fieldRect = new Rect(position);
                    fieldRect.x += EditorGUIUtility.labelWidth;
                    EditorGUI.PropertyField(fieldRect, property, GUIContent.none);
                    if (GUI.changed) property.serializedObject.ApplyModifiedProperties();

                    if (!this._foldout.Value) return;
                } else {
            ...
            if (this._buttonMethods == null)
                this._buttonMethods = new ButtonMethodHandler(property.objectReferenceValue);

            if (displayScript) position.y += position.height + 4;
            var startY = position.y - 2;
            float startX = position.x;

            var target = this.GetTargetSO(property.objectReferenceValue);
            ...
            while (propertyObject.NextVisible(propertyObject.isExpanded && !expandedReorderable)) {
                ...
                expandedReorderable = propertyObject.isExpanded
                                      && propertyObject.isArray
            ...
            if (!this._buttonMethods.TargetMethods.IsNullOrEmpty()) {
                foreach (var method in this._buttonMethods.TargetMethods) {
                    position.height = EditorGUIUtility.singleLineHeight;
                    if (GUI.Button(position, method.Name))
                        this._buttonMethods.Invoke(method.Method);
            ...
            if (this._buttonMethods.Amount > 0) bgRect.height += 5;

            this.DrawColouredRect(bgRect, new Color(.6f, .6f, .8f, .5f));
        ...
        private bool IsFolded(SerializedProperty property) {
            if (this._foldout == null) {
                this._foldout = new EditorPrefsBool("DisplayInspectorFoldout" + property.GetParent().GetType().Name + property.propertyPath);
            }
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label) {
            bool notValidType = property.propertyType != SerializedPropertyType.ObjectReference;
            bool displayScript = ((DisplayInspectorAttribute)this.attribute).DisplayScript;
            if (notValidType || property.objectReferenceValue == null || (displayScript && !this.IsFolded(property)))
                return base.GetPropertyHeight(property, label);

            if (this._buttonMethods == null)
                this._buttonMethods = new ButtonMethodHandler(property.objectReferenceValue);
            float height = displayScript
            ...
            if (this._buttonMethods.Amount > 0) height += 4 + this._buttonMethods.Amount * EditorGUIUtility.singleLineHeight;
            return height;
        }
