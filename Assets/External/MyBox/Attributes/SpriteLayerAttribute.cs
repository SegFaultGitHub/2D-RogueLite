...
namespace MyBox.Internal {
    ...
    public class SpriteLayerAttributeDrawer : PropertyDrawer {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label) {
            if (property.propertyType != SerializedPropertyType.Integer) {
                if (!this._checkedType)
                    this.PropertyTypeWarning(property);
                EditorGUI.PropertyField(position, property, label);
                return;
            }

            var spriteLayerNames = this.GetSpriteLayerNames();
            this.HandleSpriteLayerSelectionUI(position, property, label, spriteLayerNames);
        }
        ...
        private void HandleSpriteLayerSelectionUI(Rect position, SerializedProperty property, GUIContent label, string[] spriteLayerNames) {
            ...
            bool layerFound = this.TryGetSpriteLayerIndexFromProperty(out currentSpriteLayerIndex, spriteLayerNames, property);
