...
namespace MyBox.Internal {
    ...
    public class SeparatorAttributeDrawer : DecoratorDrawer {
        private SeparatorAttribute Separator => (SeparatorAttribute)this.attribute;

        public override float GetHeight() =>
            this.Separator.WithOffset
                ? 40
                : this.Separator.Title.IsNullOrEmpty()
                    ? 28
                    : 32;

        public override void OnGUI(Rect position) {
            var title = this.Separator.Title;
