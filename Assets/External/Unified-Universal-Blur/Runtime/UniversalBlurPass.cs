...
namespace Unified.UniversalBlur.Runtime
{
    internal class UniversalBlurPass : ScriptableRenderPass, IDisposable
    {
        ...
        {
            this._profilingSampler = new(k_PassName);
            this._propertyBlock = new();
        }

        public void Setup(BlurConfig blurConfig)
        {
            this._blurConfig = blurConfig;
        }
        ...
        private RenderTextureDescriptor GetDescriptor() =>
            new(this._blurConfig.Width, this._blurConfig.Height, GraphicsFormat.B10G11R11_UFloatPack32, 0)
            {
                useMipMap = this._blurConfig.EnableMipMaps,
                autoGenerateMips = this._blurConfig.EnableMipMaps
            };
        ...
        public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
        {
...
#if UNITY_6000_0_OR_NEWER
            RenderingUtils.ReAllocateHandleIfNeeded(ref this._sourceRT, descriptor, FilterMode.Bilinear, TextureWrapMode.Clamp, name: k_BlurTextureSourceName);
            RenderingUtils.ReAllocateHandleIfNeeded(ref this._destinationRT, descriptor, FilterMode.Bilinear, TextureWrapMode.Clamp, name: k_BlurTextureDestinationName);
            #else
            ...
            using (new ProfilingScope(cmd, this._profilingSampler))
            {
                ...
                {
                    BlurConfig = this._blurConfig,
                    MaterialPropertyBlock = this._propertyBlock,
                    ColorSource = colorTarget,
                    Source = this._sourceRT,
                    Destination = this._destinationRT
                }, new WrappedCommandBuffer(cmd));

                cmd.SetGlobalTexture(Constants.GlobalFullScreenBlurTextureId, this._destinationRT);
            }
        ...
        public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
        {
            ...
            var descriptor = new TextureDesc(this.GetDescriptor());
            ...
            using (var builder = renderGraph.AddUnsafePass<RenderGraphPassData>(k_PassName, out var passData, this._profilingSampler))
            {
                passData.ColorSource = cameraColorSource;
                passData.Source = source;
                passData.Destination = destination;

                passData.MaterialPropertyBlock = this._propertyBlock;
                
                passData.BlurConfig = this._blurConfig;
